export default function (searchinput, orders) {
  // determines what type of search to run and calls relevant function. The function previously known as searchHandler()
  ...
  if (parameterMatchRegex.exec(searchinput) != null) {
  ...
  switch (
    searchparameter // matches search parameter to each case and calls relevant search function
  ) {
    case 'orderid':
...
function basicSearch(searchTerm, ordersArray) {
  // basic search funtion, seaches for matches against orderid, customerid. Called when no search parameter is given, or if given parameter is invalid. eturns an array of order objects that match.
  //TODO expand search scope?
  let results = [];
  for (let i = 0; i < ordersArray.length; i++) {
    if (
      ordersArray[i].orderid == searchTerm ||
      ordersArray[i].customerid == searchTerm
    ) {
...
function orderIDSearch(searchTerm, ordersArray) {
  // called with parameter "orderid". Returns an array of order objects that match.
  let results = [];
...
function customerIDSearch(searchTerm, ordersArray) {
  // called with parameter "customerid". Returns an array of order objects that match.
  let results = [];
...
function addressSearch(searchTerm, ordersArray) {
  // called with parameter "address". Returns an array of order objects that match.
  ...
  for (let i = 0; i < ordersArray.length; i++) {
    if (ordersArray[i].invaddr == searchTerm) {
      results.push(ordersArray[i]);
    } else if (ordersArray[i].delivaddr == searchTerm) {
      //I THINK that this elif should prevent duplicates when both delivery address and invoice address are filled and the same. Please leave as-is
      results.push(ordersArray[i]);
    }
  }
  return results;
...
function deliveryDateSearch(searchTerm, ordersArray) {
  // called with parameter "date". Returns an array of order objects that match.
  let results = [];
...
function respSalesPersonSearch(searchTerm, ordersArray) {
  // called with parameter "salesperson". Returns an array of order objects that match.
  let results = [];

  for (let i = 0; i < ordersArray.length; i++) {
    if (
      ordersArray[i].respsalesperson.toUpperCase() == searchTerm.toUpperCase()
    ) {
      //TODO improve case desensitisation
      results.push(ordersArray[i]);
    }
  }
  return results;
...
function productCodeSearch(searchTerm, ordersArray) {
  // called with parameter "productid". Returns an array of order objects that match.
  let results = [];
  let productsArray = [];
  ...
  for (let i = 0; i < ordersArray.length; i++) {
    ...
    for (let j = 0; j < productsArray.length; j++) {
      if (productsArray[j].code.toUpperCase() == searchTerm.toUpperCase()) {
        //TODO improve case desensitisation
        results.push(ordersArray[i]);
      }
    }
...
function isSearch(searchTerm, ordersArray) {
  if (searchTerm === 'ready' || searchTerm === 'done') {
    return ordersArray.filter(each => each.ready);
  }

  return [];
}

function isNotSearch(searchTerm, ordersArray) {
  if (searchTerm === 'ready' || searchTerm === 'done') {
    return ordersArray.filter(each => !each.ready);
  }
